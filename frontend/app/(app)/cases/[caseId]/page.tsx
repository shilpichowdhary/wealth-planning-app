...
export default function CasePage({ params }: { params: { caseId: string } }) {
      ...
      const abort = createSSEStream(
        ...
        diagramData => {
          ...
          assistantContent = assistantContent
            .replace(/```json\s*\{[\s\S]*?"entities"[\s\S]*?\}\s*```/g, '')
            .trim()
          setMessages(prev => {
            const updated = [...prev]
            updated[updated.length - 1] = { ...updated[updated.length - 1], content: assistantContent }
  ...
  return (
    <div className="flex-1 flex flex-col h-screen max-h-screen min-h-0">
      {/* Header */}
      <header className="border-b border-ink-800 bg-ink-900/40 backdrop-blur sticky top-0 z-10">
        <div className="px-8 py-5 flex items-center justify-between gap-6">
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-[0.2em] text-ink-400 font-medium">Case</p>
            <h1 className="mt-1 font-display text-2xl font-semibold tracking-tight text-ink-100 truncate">
              {caseData?.client_name || 'Loading…'}
            </h1>
            <p className="mt-0.5 text-[11px] text-ink-500 font-mono">{caseId.slice(0, 8)}</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex rounded-lg border border-ink-700 overflow-hidden bg-ink-900">
              <button
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3.5 py-2 text-sm font-medium transition border-l border-ink-700 ${
                  activeTab === 'diagram'
                    ? 'bg-ink-800 text-ink-100'
                    : 'text-ink-300 hover:bg-ink-850 hover:text-ink-100'
                }`}
            ...
            <button
              onClick={downloadPdf}
              className="flex items-center gap-2 px-3.5 py-2 text-sm font-medium bg-ink-800 border border-ink-700 text-ink-100 rounded-lg hover:bg-ink-750 transition"
            >
              <Download size={14} />
              PDF
            </button>
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {activeTab === 'chat' ? (
          <div className="h-full flex flex-col">
            ...
            {hasPriorMemory && (
              <div className="mx-8 mt-4 flex items-center gap-2.5 rounded-lg border border-brass-500/30 bg-brass-500/[0.06] px-4 py-2.5 text-[13px] text-brass-300">
                ...
                <span>
                  ...
                  <span className="text-ink-300"> — context from previous conversations on this case is loaded.</span>
                </span>
              </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="max-w-[860px] mx-auto px-8 py-8 space-y-6">
                ...
                {messages.map(msg => (
                  ...
                  >
                    {msg.role === 'user' ? (
                      <div className="max-w-[70%] rounded-2xl rounded-br-sm bg-brass-500/[0.08] border border-brass-500/20 px-4 py-3 text-[15px] text-ink-100 leading-relaxed">
                        {msg.content}
                      </div>
                    ) : (
                      <div className="w-full">
                        <div className="mb-2 flex items-center gap-2 text-[11px] uppercase tracking-[0.16em] text-ink-400 font-medium">
                        ...
                        {msg.sources && msg.sources.length > 0 && (
                          <div className="mt-4 pt-4 border-t border-ink-800">
                            <p className="text-[11px] uppercase tracking-[0.16em] text-ink-400 font-medium mb-2">Sources</p>
            ...
            {/* Composer */}
            <div className="border-t border-ink-800 bg-ink-900/40 backdrop-blur">
              <div className="max-w-[860px] mx-auto px-8 py-5">
                ...
                <div className="relative rounded-2xl border border-ink-700 bg-ink-850 focus-within:border-lc-red/60 focus-within:ring-2 focus-within:ring-lc-red/20 transition">
                  <textarea
                    value={input}
                    ...
                    rows={2}
                    disabled={streaming}
                    className="w-full bg-transparent px-4 py-3.5 text-[15px] text-lc-white placeholder:text-ink-500 focus:outline-none resize-none disabled:opacity-50 pr-14"
                  />
                  <button
                    onClick={() => sendMessage()}
                    disabled={streaming || !input.trim()}
                    className="absolute bottom-2.5 right-2.5 inline-flex h-9 w-9 items-center justify-center rounded-lg bg-lc-red text-lc-white hover:bg-lc-red/90 transition disabled:opacity-40 disabled:cursor-not-allowed"
                    title="Send (Enter)"
                  >
                    <Send size={14} />
                  </button>
          ...
          <div className="h-full bg-lc-black">
            {saveError && (
...
function KbPermissionModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-lc-black/75 backdrop-blur-sm animate-fade-in-up">
      <div className="relative w-full max-w-lg rounded-2xl border border-ink-700 bg-ink-900 p-6">
        <button
          onClick={onDismiss}
          className="absolute top-3 right-3 p-1.5 rounded-md text-ink-400 hover:text-lc-white hover:bg-ink-800 transition"
          title="Dismiss"
        >
          <X size={14} />
        </button>
        ...
        <h2 className="font-display text-2xl text-lc-white leading-snug">
          Your KB returned {evt.kb_chunk_count} {evt.kb_chunk_count === 1 ? 'match' : 'matches'} for this
          question.
        </h2>
        <p className="mt-2 text-[14px] text-ink-300 text-pretty">
        ...
        {evt.kb_preview.length > 0 && (
          <div className="mt-4 rounded-lg border border-ink-700 bg-ink-850 p-3">
            <p className="text-[10px] uppercase tracking-[0.16em] text-ink-400 font-bold mb-2">Closest KB matches</p>
            <ul className="space-y-1">
              {evt.kb_preview.map((p, i) => (
                <li key={i} className="text-[12px] text-ink-200 flex items-center gap-2">
                  <span className="text-ink-400 tabular-nums">{(p.similarity ?? 0).toFixed(2)}</span>
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button
            onClick={onSearchWeb}
            className="inline-flex items-center justify-center gap-2 rounded-lg bg-lc-red text-lc-white py-3 text-sm font-bold hover:bg-lc-red/90 transition"
          >
            <Globe size={14} />
            Search the web
          </button>
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-lg bg-ink-850 border border-ink-700 text-ink-200 py-3 text-sm font-bold hover:bg-ink-800 hover:text-lc-white transition"
          >
        ...
        <p className="mt-4 text-[11px] text-ink-400 leading-relaxed">
          <strong className="text-ink-200">Tip:</strong> if the topic should be covered by the KB, consider
...
function WelcomePrompt({ onSuggest }: { onSuggest: (s: string) => void }) {
  return (
    <div className="py-12 animate-fade-in-up">
      ...
      <h3 className="font-display text-2xl font-semibold text-ink-100 tracking-tight">How can I help?</h3>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-2">
        {SUGGESTIONS.map(s => (
          <button
            key={s}
            onClick={() => onSuggest(s)}
            className="text-left px-4 py-3 rounded-lg border border-ink-800 bg-ink-900/50 hover:bg-ink-850 hover:border-ink-700 transition text-[13px] text-ink-200"
          >
            {s}
          </button>
        ))}
