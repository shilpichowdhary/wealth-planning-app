...
const defaultEdgeOptions = {
  ...
  animated: false,
  style: { stroke: '#CCC6BE', strokeOpacity: 0.6, strokeWidth: 1.5 },
  labelBgStyle: { fill: '#000000', fillOpacity: 0.9 },
  labelBgPadding: [6, 4] as [number, number],
  labelBgBorderRadius: 4,
  labelStyle: { fill: '#CCC6BE', fontSize: 11, fontWeight: 600 },
  markerEnd: { type: MarkerType.ArrowClosed, color: '#CCC6BE', width: 16, height: 16 },
}
...
export function StructureDiagram({ diagramData, title, emptyLabel, onSave, savedAt }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-lc-black">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-ink-800 bg-ink-900/40">
        <div className="flex items-center gap-3 min-w-0">
          <h3 className="text-[11px] uppercase tracking-[0.16em] font-bold text-ink-200">{title}</h3>
          {hasData && (
            ...
            <>
              ...
              <button
                ...
                className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-bold transition ${
                  !dirty
                    ? 'border border-ink-700 bg-ink-900 text-ink-400 cursor-default'
                    : 'border border-lc-red bg-lc-red text-lc-white hover:bg-lc-red/90'
                }`}
        ...
        {hasData ? (
          <>
            <ReactFlow
              ...
              nodesDraggable={true}
            >
              <Background color="rgba(204,198,190,0.12)" gap={24} size={1} />
              <Controls
                showInteractive={false}
                className="!bg-ink-900 !border-ink-700 !rounded-lg [&>button]:!bg-ink-900 [&>button]:!border-ink-700 [&>button]:!text-ink-300 [&>button:hover]:!bg-ink-800"
              />
            </ReactFlow>
            <div className="pointer-events-none absolute bottom-2 right-2 text-[10px] text-ink-500 bg-ink-900/80 border border-ink-700 rounded px-2 py-1 backdrop-blur">
              Double-click to rename · drag handle to connect · Del to remove
            </div>
          </>
        ) : (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
            <p className="text-sm text-ink-400">{emptyLabel ?? 'No diagram yet.'}</p>
...
function ToolbarButton({
  ...
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-1 rounded-md border border-ink-700 bg-ink-900 px-2 py-1 text-[11px] font-bold text-ink-200 hover:bg-ink-800 hover:text-lc-white hover:border-ink-600 transition"
    >
      <Plus size={11} className="text-ink-400" />
      <Icon size={11} />
      {label}
    </button>
  )
